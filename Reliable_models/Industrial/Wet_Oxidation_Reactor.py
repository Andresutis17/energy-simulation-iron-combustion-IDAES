"""
Wet oxidation BFB reactor at INDUSTRIAL scale  (non isothermal)
"""
import sys, os
...
def main():
    ...
    if terminal_condition == "optimal":
        ...
        # Axial profile: rates, C_H2O, solid composition along the bed
        H_bed = value(blk.bed_height)
        fe_in_ax = (value(blk.solid_inlet.flow_mass[0])
                    * value(blk.solid_inlet.mass_frac_comp[0, "Fe"]) / MW["Fe"])
        print(f"\nAxial profile:")
        for x in m.fs.BFB.length_domain:
            z = x * H_bed # x: IDAES normalized axial coordinate. z: distributor height
            r1 = value(blk.solid_emulsion.reactions[0, x].reaction_rate["OW1"])
            r2 = value(blk.solid_emulsion.reactions[0, x].reaction_rate["OW2"])
            c_h2o = value(blk.gas_emulsion.properties[0, x].dens_mol_comp["H2O"])
            ...
            xfe = ((fe_in_ax - value(sp.flow_mass) * w_fe / MW["Fe"])
                   / fe_in_ax * 100 if fe_in_ax > 0 else 0.0)
