...
def main():
   # Operating variables
    ...
    if terminal_condition == "optimal":
        ...
        # Axial profile: rates, C_H2, solid composition along the bed
        H_bed = value(blk.bed_height)
        fe2o3_in_ax = (value(blk.solid_inlet.flow_mass[0])
                       * value(blk.solid_inlet.mass_frac_comp[0, "Fe2O3"])
                       / MW["Fe2O3"])
        print(f"\nAxial profile:")
        for x in m.fs.BFB.length_domain:
            z = x * H_bed # x: IDAES normalized axial coordinate. z: distributor height
            r1 = value(blk.solid_emulsion.reactions[0, x].reaction_rate["RD1"])
            r2 = value(blk.solid_emulsion.reactions[0, x].reaction_rate["RD2"])
            r3 = value(blk.solid_emulsion.reactions[0, x].reaction_rate["RD3"])
            c_h2 = value(blk.gas_emulsion.properties[0, x].dens_mol_comp["H2"])
            sp = blk.solid_emulsion.properties[0, x]
            ...
            xfe2o3 = ((fe2o3_in_ax - value(sp.flow_mass) * w_fe2o3 / MW["Fe2O3"])
                      / fe2o3_in_ax * 100 if fe2o3_in_ax > 0 else 0.0)
