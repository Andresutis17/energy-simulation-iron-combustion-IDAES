"""
Custom solid phase property package for iron-energy CLC modeling.
Components - Fe2O3, Fe3O4, FeO, Fe, Al2O3, CuO

Extends IDAES oxygen_iron_OC_oxidation solid phase by adding FeO and Fe.


References:
(1) NIST Chemistry WebBook, https://webbook.nist.gov/chemistry/
(2) Barin, I., Thermochemical Data of Pure Substances, VCH, 1993.
(3) NIH PubChem, https://pubchem.ncbi.nlm.nih.gov
"""
...
@declare_process_block_class("CustomSolidPhaseParameterBlock")
class PhysicalParameterData(PhysicalParameterBlock):
    """
    Property Parameter Block Class
    Contains parameters for extended solid phase with Fe2O3, Fe3O4, FeO, Fe and Al2O3.
    ...
    def build(self):
        ...
        self._state_block_class = CustomSolidPhaseStateBlock
        ...
        # Mol. weights - kg/mol - ref: NIST webbook
        mw_comp_dict = {
            "Fe2O3": 0.15969,
            "Fe3O4": 0.231533,
            "FeO": 0.071844,
        ...
        # Skeletal density of solid components - units = kg/m3. ref: NIST, Pubchem
        ...
        # cp_comp = (kJ/mol.K)
        # H_comp = H - H(298.15) = A*T + B*T^2/2 + C*T^3/3 + D*T^4/4 - E/T + F - H   
        # H_comp = (kJ/mol)
        cp_param_dict = {
            ("Al2O3", 1): 102.4290,
            ("Al2O3", 2): 38.74980,
        ...
        self.cp_param_8 = Param(
            ...
            doc="Shomate cp coeff 8",
            units=pyunits.kJ / pyunits.mol,
        )

        # Std. heat of formation - J/mol - ref: NIST
        ...
        # Set default scaling for thermophysical properties
...
class _CustomSolidPhaseStateBlock(StateBlock):
    """
    Methods applied to State Blocks as a whole.
            ...
            for k in blk.values():
                if degrees_of_freedom(k) != 0:
                    ...
                    raise Exception(
                        ...
                        "for state block is not zero during initialization."
                    )
...
@declare_process_block_class(
    "CustomSolidPhaseStateBlock", block_class=_CustomSolidPhaseStateBlock
)
class SolidPhaseStateBlockData(StateBlockData):
    """
    Solid phase state block for extended iron-energy CLC.
    """
    ...
    def calculate_scaling_factors(self):
        ...
        if self.is_property_constructed("sum_component_eqn"):
            # Scaling factor using Fe2O3 as reference works for all reactors
            # because all mass fractions are bounded in [0,1]. This does not
            # affect physical results, only numerical conditioning for IPOPT.
